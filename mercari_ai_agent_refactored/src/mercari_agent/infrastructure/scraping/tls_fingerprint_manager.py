...
class TLSFingerprint:
    ...
    usage_count: int = 0
    fingerprint_id: str = field(default="")
    
    def __post_init__(self):
        """初始化后处理"""
        if not self.fingerprint_id:
            # 生成指纹ID
            fingerprint_data = f"{self.ja3_hash}_{self.ja4_hash}_{self.browser_type.value}_{self.tls_version.value}"
            self.fingerprint_id = hashlib.md5(fingerprint_data.encode()).hexdigest()
...
class TLSFingerprintManager:
    ...
    def create_ssl_context(self, fingerprint: TLSFingerprint) -> ssl.SSLContext:
        ...
        # 设置TLS版本
        context.minimum_version = self.config.min_tls_version.to_ssl_version()
        context.maximum_version = self.config.max_tls_version.to_ssl_version()
